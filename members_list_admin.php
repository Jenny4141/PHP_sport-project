  ...
  <div class="row justify-content-center">
    <div class="col-12 col-sm-11 col-md-10">
      <div class="d-flex flex-wrap justify-content-md-between align-items-center gap-2 mb-3">
        ...
        <!-- 搜尋框區塊 -->
        <form class="d-flex align-items-center mb-0" role="search" method="GET" action="members_list.php">
      ...
      <table class="mb-0 table table-bordered table-striped text-center text-break align-middle">
        <thead>
          <tr id="member_tr">
            ...
            <th>大頭貼</th>
            <th>收件地址</th>
            <!-- <th class="status sm-th">是否啟用帳號</th>
            <th class="status sm-th">是否驗證信箱</th>
            <th class="status sm-th">是否驗證電話</th> -->
            <!-- <th>使用者角色</th> -->
            ...
            <tr>
              ...
              <td>
                <?php if (!empty($r['avatar_url'])): ?>
                  <img src="<?= htmlspecialchars($r['avatar_url']) ?>" class="rounded-circle" style="max-width: 60px;">
                <?php endif; ?>
              </td>
              ...
              <td>
                <a href="members_edit.php?id=<?= $r['id'] ?>">
        ...
        fetch(`members_delete-api.php?page=${page}&search=${encodeURIComponent(search)}`, {
          ...
          .then(data => {
            if (data.success) {
              ...
              // 刪除後更新 URL，停留在當前分頁
              window.location.href = `members_list.php?page=${page}&search=${encodeURIComponent(search)}`;
            } else {
