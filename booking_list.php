...
if (!empty($search)) {
  $searchSql = "WHERE m.full_name LIKE ? OR c.classname_id LIKE ? OR bs.status_name LIKE ?";
  ...
  <div class="row justify-content-center">
    <div class="col-12 col-sm-11 col-md-10">
      <div class="d-flex flex-wrap justify-content-md-between align-items-center gap-2 mb-3">
        <a href="booking_add.php" class="btn btn-outline-primary">新增</a>
        <form class="d-flex align-items-center mb-0" role="search" method="GET" action="booking_list.php">
          <input class="form-control me-2" type="search" name="search" placeholder="輸入學員名、單號" value="<?= htmlentities($search) ?>" aria-label="Search" />
          ...
          <table class="mb-0 table table-bordered table-striped text-center text-break align-middle">
            ...
            <tbody>
              <?php foreach ($rows as $r): ?>
                <tr>
                  <td class="sm-td"><?= $r['booking_id'] ?></td>
                  <td class="search-field "><?= htmlentities($r['member_name']) ?></td>
                  <td class="search-field "><?= htmlentities($r['course_name']) ?></td>
                  <td class="search-field "><?= htmlentities($r['booking_status']) ?></td>
                  ...
                  <td class="search-field "><?= htmlentities($r['create_time']) ?></td>
                  <td class="sm-td">
                    <a href="booking_edit.php?id=<?= $r['booking_id'] ?>">
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    confirmDeleteButton.addEventListener("click", () => {
      if (deleteId) {
        ...
        fetch(`booking_delete-api.php?page=${page}&search=${encodeURIComponent(search)}`, {
            method: "POST",
            headers: {
          ...
          .then(data => {
            if (data.success) {
              ...
              window.location.href = `booking_list.php?page=${page}&search=${encodeURIComponent(search)}`;
            } else {
